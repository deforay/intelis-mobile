...
export class EnterAppPasswordPage implements OnInit {
  ...
  ionViewWillEnter() {
    ...
    // });
    console.log('this.isBiometricUsed', this.isBiometricUsed);
  }
  ...
  async onPinNumberChange(pin) {
    ...
    if (!isValid) {
      const controlName = 'ctrl_' + (pin.length - 1).toString();
