  ...
  public getSQLLiteCovid19Data() {
    ...
    this.sql.create({
      ...
      return new Promise((resolve, reject) => {
        db.executeSql('SELECT * FROM form_covid19 where form_covid19.user_id=?', [this.userID]).then(async data => {
          ...
          console.log(this.userTestPendingResultArray, 'Covid-19 new');
  ...
  public async filterItems() {
    ...
    if (this.searchTerm && this.searchTerm.trim() !== '') {
      ...
      filteredArray = filteredArray.filter(item => 
        (item.patient_id && item.patient_id.toString().toLowerCase().includes(searchTermLower)) ||
        (item.patient_name && item.patient_name.toString().toLowerCase().includes(searchTermLower)) ||
