...
export class ViewVlPage implements OnInit {
  ...
  constructor(private changeDetectorRef: ChangeDetectorRef,private storage: Storage, public popoverController: PopoverController, public sql: SQLite, public events: Events) { }
  ...
  async ionViewWillEnter() {
    ...
    await this.storage.set('previousPageUrl', '/view-vl');
    await this.getSQLLiteVlData();
    this.events.subscribe('isSyncWithVl', async (result: any) => {
      if (result == true) {
        await this.getSQLLiteVlData();
      }
    });
  ...
  async getSQLLiteVlData() {
    ...
    await this.sql.create({
      ...
      return new Promise((resolve, reject) => {
        db.executeSql('SELECT * FROM vl_request_form where user_id=? ORDER BY last_modified_datetime DESC', [this.userID]).then(async data => {
  ...
  async searchUserTestRequests() {
    ...
    if (this.searchTerm && this.searchTerm.trim() !== '') {
      ...
      filteredArray = filteredArray.filter(item =>
        (item.patient_art_no && item.patient_art_no.toLowerCase().includes(searchTermLower)) ||
        (item.patient_first_name && item.patient_first_name.toLowerCase().includes(searchTermLower)) ||
  ...
  //               return (a.sampleCode.substring(aSampleCodeLength - 3)) -
  ...
  //               let aSampleCodeLength = a.sampleCode.length;
  //               let bSampleCodeLength = b.sampleCode.length;
  //               return (b.sampleCode.substring(bSampleCodeLength - 3)) -
