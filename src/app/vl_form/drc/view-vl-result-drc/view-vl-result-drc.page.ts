...
export class ViewVlResultDrcPage implements OnInit {
  ...
  async ionViewWillEnter() {
    ...
    await this.storage.get('initArray').then(async (initArray) => {
      if (initArray) {
        this.facilitiesArray = initArray.testingLabsList.filter(item => item.test_type === 'vl');
        this.sampleStatusArray = initArray['vl'].statusFilterList;
      }
    })
  ...
  async searchRecord() {
    ...
    if (this.searchRecordForm.controls.sampleCode.value == '' && this.searchRecordForm.controls.patientID.value == '' && this.searchRecordForm.controls.patientName.value == '' && this.searchRecordForm.controls.facilities.value == '' && this.searchRecordForm.controls.sampleStatus.value == '') {
      ...
      console.log(this.searchRecordForm.controls.sampleCode.value,this.searchRecordForm.controls.patientID.value,this.searchRecordForm.controls.patientName.value,this.searchRecordForm.controls.facilities.value,this.searchRecordForm.controls.sampleStatus.value,'this.searchRecordForm.controls.value');

      if(this.searchRecordForm.controls.sampleStatus.value == ''){
        // this.searchRecordForm.get('sampleStatus').setValue(null);
      ...
      let searchJSON = {
        ...
        "patientArtNo": this.searchRecordForm.controls.patientID.value ? [this.searchRecordForm.controls.patientID.value] : [],
      ...
      await this.CrudService.postDataWithLoader('/api/v1.1/vl/fetch-results.php', searchJSON, this.authToken, true).then(async (result) => {
        ...
        if (result['status'] == 'success') {
          ...
          console.log(result['data'],this.searchArray,'SearchArray');
          this.searchSortingArray = result['data'];
          console.log(this.searchSortingArray,'searchSortingArray');
  ...
  async presentSortingPopover(ev: any) {
    ...
    popover.onDidDismiss()
      .then((result) => {
        ...
        console.log(result['data'],'sortedData');
        if (result['data']) {
          ...
          if (this.sortedData) {
            ...
            if (this.sortedData.order == 'asc' && this.sortedData.value == 'lastModified') {
              ...
              this.searchArray = this.searchSortingArray.sort(function (a, b) {
                return new Date(a.lastModifiedDatetime ? a.lastModifiedDatetime : a.requestCreatedDatetime).getTime() - new Date(b.lastModifiedDatetime ? b.lastModifiedDatetime : b.requestCreatedDatetime).getTime()
              });
            } else if (this.sortedData.order == 'des' && this.sortedData.value == 'lastModified') {
              this.searchArray = this.searchSortingArray.sort(function (a, b) {
                return new Date(b.lastModifiedDatetime ? b.lastModifiedDatetime : b.requestCreatedDatetime).getTime() - new Date(a.lastModifiedDatetime ? a.lastModifiedDatetime : a.requestCreatedDatetime).getTime()
              });
